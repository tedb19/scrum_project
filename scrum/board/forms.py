...
'''
TaskFilter allows the client to filter on the sprint,
status, or assigned user.
However, the sprint for a task isn’t required and this
filter won’t allow for selecting tasks without a sprint.
In our current data model, a task that isn’t currently
assigned a sprint would be considered a backlog task.
To handle this, we add a new field to the TaskFilter
called backlog that uses the NullFilter
This will make http://localhost:8000/api/tasks/?backlog=True
return all tasks that aren’t assigned to a sprint.

Another issue with TaskFilter is that the users referenced by
assigned are referenced by the pk, while the rest of the API
uses the username as a unique identifier.
...
retrived using http://localhost:8000/api/tasks/?assigned=demo
rather than http://localhost:8000/api/tasks/?assigned=1.
'''
